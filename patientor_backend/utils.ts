...
const parseStringObject=(input:unknown):string=>{
    if(!input || !isString(input))throw new Error("Incorrect or missing string object: "+input);
    else if(input.length <3) throw new Error ("Each field requires a minimum lentgh of 3 charracters")
    return input;
};
...
const parseTreatmentCategory=(category:unknown): ("Hospital"|"OccupationalHealthcare"|"HealthCheck")=>{
    if(!category || !isString(category) || !isTreatmentCategory(category)) throw new Error('Incorrect or missing treatment category: '+category);
    return category;
};
const parseDate=(date:unknown, item:string):string=>{
    if(!date || !isString(date) || !isDate(date)) throw new Error("incorrect or missing date info: "+date+item);
    return date;
};
...
export const toNewDiagnosesInput = (entry: unknown):Diagnosis => {
...
const toDischarge = (obj: unknown): Discharge => {
    if(!obj || typeof obj !== 'object') throw new Error ("incorrect or missing discharge date " + obj);
    if("date" in obj && "criteria" in obj){
        const discharge : Discharge = {
            date : parseDate (obj.date, "Discharge Date"),
...
const toSickLeave = (obj: unknown): SickLeave => {
    ...
    if("startDate" in obj && "endDate" in obj){
        const sickLeave : SickLeave ={
            startDate: parseDate(obj.startDate, "Sick Leave Start Date"),
            endDate: parseDate(obj.endDate, "Sick Leave End Date"),
        };
        return sickLeave;
...
export const parseDiagnosesCode = (param: unknown): Diagnosis["code"] => {
    if(!param || typeof param !== "string" || param==="")throw new Error("Error in new diagnose code: code doesn't exists or is not a correct type");
    return param;
};

export const toFullPatientInput = (obj:unknown): Patient => {
...
export const toNewPatientInput = (obj:unknown):Patient => {
    if(!obj || typeof obj !=='object'){ throw new Error ('toNewPatientInpu: incomplete or missing data'); }
...
const toTreatmentEntry = (obj:unknown): Entry => {
    ...
    "treatment" in objValues){
        ...
        return entry; 
    }throw new Error (" Treatment data incorrect or missing");
};

export const toNewTreatmentEntry = (obj:unknown): Entry => {
    if (!obj||typeof obj !=='object')  throw new Error ('utils>treatment does not exists or is not an object');
    const objValues = obj;

    if("date" in objValues &&
    "specialist" in objValues &&
    "description" in objValues &&
    "treatment" in objValues){
        const entry :Entry ={
        ...
        return entry; 
    }throw new Error ("Entry form is incomplete- required fields are missing");
};
